  ...
  return (
    <div className='pt-32 p-10 min-h-screen bg-linear-to-b from-[#faf6ef] to-[#e8ddc9]'>
      ...
      <motion.button whileHover={{scale:1.07}} whileTap={{scale:0.95}} onClick={()=>openForm()}
        className='flex items-center gap-3 bg-[#C9A86A] text-white px-6 py-3 
      ...
      <div className='mt-8 grid md:grid-cols-2 lg:grid-cols-3 gap-6'>
        {customers.map((c)=>(
          ...
          whileHover={{scale:1.03}} className='p-6 bg-white/90 backdrop-blur-xl
          border border-[#C9A86A]/25 rounded-2xl shadow-md hover:shadow-xl transition-all'>
            <div className='mb-4'>
              <p className='text-xl font-semibold text-neutral-800'>{c.name}</p>
              <p className='text-neutral-600'>{c.email}</p>
            </div>
            <div className='flex justify-end gap-4'>
              <button onClick={()=> openForm(c)} className='text-[#C9A86A] hover:text-[#a38552]'>
                <FaEdit size={20}/>

              </button>
              <button onClick={()=> deleteCustomer(c._id)} className='text-red-600 hover:text-red-800'>
                <FaTrash size={20}/>
        ...
        className='fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center p-4'>
          ...
          className='bg-white/95 p-7 rounded-3xl w-full max-w-sm shadow-2xl border border-[#C9A86A]/40'>
            ...
            <div className='flex justify-between mt-6'>
              ...
              className='px-4 py-2 bg-[#C9A86A] rounded-xl font-semibold shadow-md hover:shadow-lg transition'>
                {isEdit ? 'Update':'Add'}
