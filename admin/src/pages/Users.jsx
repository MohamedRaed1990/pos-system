...
const Users = () => {
  ...
  return (
    <div className='p-6 bg-linear-to-b from-[#faf6ef] to-[#e8ddc9] pt-32 h-screen mx-auto'>
      ...
      <button onClick={()=> handleOpenModal()} className='mb-6 px-5 py-2 bg-blue-600 text-white
      rounded-xl flex items-center gap-2 hover:bg-blue-700 transition active:scale-95 shadow-md'>
        <FiPlus/> Add User
      </button>
      <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6'>
        {users.map((user,index)=>(
          <motion.div key={user._id} initial={{opacity:0,y:15}} animate={{opacity:1,y:0}}
          transition={{delay:index*0.06}} className='bg-white border border-gray-200
      ...
      <AnimatePresence>
        ...
        className='fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-100'>
            <motion.div initial={{scale:0.05,opacity:0}} animate={{scale:1,opacity:1}} exit={{scale:0.05,opacity:0}}
            className='bg-white rounded-2xl p-6 w-full max-w-lg shadow-xl'>
              ...
              <div className='space-y-4'>
                ...
                placeholder='Name' value={currentUser.name}
                ...
                <input className='w-full p-3 border rounded-lg' 
                placeholder='Role (admin / user)' value={currentUser.role}
                onChange={(e)=> setCurrentUser({...currentUser,role:e.target.value})}/>

                <input className='w-full p-3 border rounded-lg' 
                placeholder='Password' value={currentUser.password}
