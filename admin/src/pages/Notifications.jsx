...
const Notifications = () => {
  ...
  const handleMarkSeen = async(id)=>{
    try{
      ...
      setNotifications((prev)=> prev.map((note)=> (note._id === id ? res.data : note)))
    }catch(err){
      console.error(err)
    }
  }
    ...
    <div className='p-6 bg-linear-to-b from-[#faf6ef] to-[#e8ddc9] pt-32 h-screen mx-auto'>
        ...
        {notifications.map((note)=>{
          ...
          switch (note.type){
            case 'transaction' :
              ...
              break;
            case 'project' :
              icon = <FiAlertTriangle size={22} className='text-green-600'/>
              styles= 'bg-blue-50 border-blue-300';
              break;
            default :
              icon = <FiInfo size={22} className='text-gray-600'/>
              styles= 'bg-green-50 border-green-300';
              break;    
          }
          return (
            <motion.div key={note._id} 
            initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} layout
            className={`flex items-center justify-between p-4 mb-3 rounded-xl
            border shadow-sm hover:shadow-sm transition ${styles} `}>
      ...
      {openModal && (
        <div className='fixed inset-0 flex justify-center items-center bg-black/40'>
          <motion.div initial={{opacity:0, scale:0.8}} animate={{scale:1,opacity:1}}
